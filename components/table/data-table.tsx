...
export function DataTableCellPill({
  ...
  if (children === null || children === undefined || children === "") {
    return <span className="text-[var(--text-muted)] tabular-nums">{empty}</span>;
  }
  return (
    <span
      className={cn(
...
export function DataTable<T>({
  ...
  const totalRows = isServer ? serverPagination.totalRows : (totalRowsProp ?? data.length);
  ...
  return (
    ...
    >
      <div className="flex flex-wrap items-center justify-between gap-2 border-b border-[var(--border)] px-3 py-2 sm:px-4">
        <div className="min-w-0 flex-1 text-xs font-medium text-[var(--text-h)] sm:text-sm">{toolbar}</div>
        {primaryAction ? (
          <Button
            type="button"
            variant="outlined"
        ...
        <TableBody>
          {pageRows.length > 0 ? (
            pageRows.map((row, i) => {
              ...
              const key = getRowId ? getRowId(row, globalIndex) : globalIndex;
              return (
                <TableRow
                  key={String(key)}
                ...
                >
                  {columns.map((col) => (
                    <TableCell
                      key={col.id}
                      className={cn("text-[var(--text)] first:text-[var(--text-h)]", col.cellClassName)}
                    >
            ...
            <TableRow>
              <TableCell colSpan={columns.length} className="h-32 p-6">
                <div className="flex flex-col items-center justify-center gap-2 text-center">
                  <IoArchive className="size-9 text-[var(--text-muted)] opacity-70" aria-hidden />
                  <p className="text-sm font-medium text-[var(--text-h)]">{emptyTitle}</p>
                  <p className="max-w-md text-xs leading-relaxed text-[var(--text-muted)]">{emptyDescription}</p>
                </div>
              </TableCell>
            </TableRow>
          ) : (
            <TableRow>
              <TableCell colSpan={columns.length} className="h-16 text-center text-xs text-[var(--text-muted)]">
                —
              </TableCell>
            </TableRow>
          )}
      ...
      <div className="flex flex-col gap-2 border-t border-[var(--border)] bg-[color-mix(in_srgb,var(--social-bg)_35%,var(--bg-elevated))] px-3 py-2 sm:flex-row sm:items-center sm:justify-between sm:px-4 dark:bg-white/[0.03]">
        ...
        <div className="flex flex-wrap items-center justify-end gap-2 sm:gap-3">
          <div className="flex items-center gap-1.5">
            <span className="whitespace-nowrap text-[11px] text-[var(--text-muted)]">{rowsPerPageLabel}</span>
            <select
          ...
          <div className="flex items-center gap-0.5">
            ...
            >
              <IoPlayBack className="size-3.5" aria-hidden />
            </Button>
            <Button
              type="button"
              variant="outlined"
            ...
            >
              <IoPlayForward className="size-3.5" aria-hidden />
            </Button>
          </div>
        </div>
      </div>
