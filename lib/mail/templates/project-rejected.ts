...
export function projectRejectedMailContent(input: {
  projectName: string;
  projectsUrl: string;
}): TMailContent {
  ...
  return {
    subject: `Your ${app} Project Was Rejected`,
    text: [
      "Hello!",
      "",
      `"${input.projectName}" was rejected on ${app}.`,
      "",
    ...
    html: renderMailLayout({
      ...
      previewText: `${input.projectName} was rejected on ${app}.`,
      bodyHtml: mailBodyHtml(
        "Hello!",
        `${mailStrong(input.projectName)} was rejected on ${escapeHtml(app)}.`,
        "Open your projects page to review the project status.",
      ),
