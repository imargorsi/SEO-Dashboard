...
export type RolesTableFeedback = {
...
type RolesTableProps = {
  initialFeedback?: RolesTableFeedback | null
}

export function RolesTable({ initialFeedback = null }: RolesTableProps) {
  ...
  const { canCreate, canUpdate, canDelete, canViewDetail } = useMemo(() => {
    ...
    return {
      ...
      canDelete: roleCanDelete(p),
      canViewDetail: roleCanView(p),
    }
  }, [authUser])
  ...
  const [feedback, setFeedback] = useState<RolesTableFeedback | null>(initialFeedback)
  ...
  const { data, error, isLoading, isValidating, mutate } =
    useSWR<AdminRolesResponse>(swrKey)
  ...
  const rows: RoleTableRow[] = useMemo(() => {
    if (!data?.success) return []
  ...
  const goToEdit = useCallback(
    (row: RoleTableRow) => {
      navigate(`/roles/${row.id}/edit`, { state: { role: row } })
    },
    [navigate],
  )
  ...
  const confirmDelete = useCallback(async () => {
    ...
    try {
      const json = await api.delete<AdminRoleDeleteResponse>(
        adminRolePath(deleteTarget.id),
      )
      const apiMsg =
  ...
  const columns = useMemo<DataTableColumn<RoleTableRow>[]>(() => {
    const cols: DataTableColumn<RoleTableRow>[] = [
      {
        id: "id",
      ...
      },
      {
        ...
        cell: (row) => (
          <DataTableCellPill>{String(row.permissionsCount)}</DataTableCellPill>
        ),
      },
    ]
      ...
      >
        <SheetContent side="right" className="gap-0 overflow-hidden p-0 sm:max-w-md">
          ...
          <div className="flex max-h-[calc(100vh-8rem)] flex-col gap-5 overflow-y-auto px-5 py-5">
            ...
            <DetailField label={t("table.colPermissions")}>
              {detailRow && detailRow.permissions.length ? (
                <div className="flex flex-wrap gap-1.5">
                  {detailRow.permissions.map((perm) => (
                    <span
                      key={perm}
